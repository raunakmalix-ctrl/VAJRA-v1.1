...
if [ -d "$CF" ]; then
  ...
  # basicsr/__init__.py does `from .version import __gitsha__, __version__`,
  # but that file is normally auto-generated by `python setup.py develop` —
  # which we never run (we just clone + PYTHONPATH it). Stub it so the import
  # succeeds ("ModuleNotFoundError: No module named 'basicsr.version'").
  if [ ! -f "$CF/version.py" ]; then
    printf '__version__ = "1.4.2"\n__gitsha__ = "unknown"\nversion_info = (1, 4, 2)\n' \
      > "$CF/version.py"
    echo "stubbed CodeFormer basicsr/version.py"
  fi
